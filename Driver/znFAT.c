#include "znfat.h"
#include "msd.h"        //存储设备的扇区读写驱动，这里是SD卡
#include "usb.h"	   //存储设备的扇区读写驱动，这里是U盘
//#include "cf.h"
#include "string.h"
...
UINT8 znFAT_ReadSector(UINT32 addr,UINT8 *buf) 
{
 switch(Dev_No)	//由Dev_No来决定所使用的存储设备驱动
 {
  case SDCARD:
  addr<<=9;
  return MSD_ReadBlock(buf, addr, 512);//SD_Read_Sector(addr,buf);
         break;
  case UDISK:
...
UINT8 znFAT_WriteSector(UINT32 addr,UINT8 *buf)
{
 switch(Dev_No)	//由Dev_No来决定所使用的存储设备驱动
 {
  case SDCARD:
  addr<<=9;
  return MSD_WriteBlock(buf, addr, 512);//SD_Write_Sector(addr,buf);
         break;
  case UDISK:
...
/******************************************************************
 ...
 - 注：有些存储设备格式化为znFAT以后，没有MBR，则0扇区就是DBR
       如果有MBR，就需要对其进行解析，以得到DBR的扇区位置，同时MBR中
       还含分区、分区容量等信息
 ******************************************************************/

UINT8 znFAT_is_MBR(void)
{
 UINT8 result;
...
UINT32 znFAT_Get_Total_Size(void) 
{
 ...
 temp=((LE2BE((((struct znFAT_BPB *)(znFAT_Buffer))->BPB_TotSec32),4)))*pArg->BytesPerSector;
 return temp;
}
...
void znFAT_Init(void)
{
 ...
 pArg->Total_Size      =znFAT_Get_Total_Size();         //znFAT_Get_Total_Size()可以返回磁盘的总容量，单位是字节
