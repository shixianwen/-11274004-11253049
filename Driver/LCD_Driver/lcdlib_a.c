...
void LCD_Copy(u32 *pbuf,u32 x0,u32 y0, u32 x1, u32 y1);
void LCD_Point(u32 x,u32 y,u32 color);
...
//internal 
void _HLine(int y, int x0, int x1, u32 color);
void _VLine(int x, int y0, int y1, u32 colorx);
//void _OutLine(int c0,int c, char *d);
// internal use
...
void LCD_ClearScr(u32 color)
{
    ...
    // fill the cache with given color
    while(i--){
      *p++=color;
    }
...
// -----------------------------------------------------------------
// gSetColor : 设定文字输出的前景颜色
// input : 
//    color : 32-bit color pattern
// -----------------------------------------------------------------
void gSetColor(u32 color)
{
	dc.color=color;
}
// -----------------------------------------------------------------
// gSetBkColor : 设定文字输出的背景颜色
// input : 
//    color : 32-bit color pattern
// -----------------------------------------------------------------
void gSetBkColor(u32 color)
{
	dc.backcolor=color;
}
// -----------------------------------------------------------------
// gSetBkMode : set background mode设 定文字与背景反转模式
// input : 
//    bkmode : =1 : use background color, =0: do not use back color
// -----------------------------------------------------------------
void gSetBkMode(u32 bkmode)
{
	dc.textbkmode=bkmode;
}
// -----------------------------------------------------------------
...
{ 
  _HLine(y0,x0,x1,color);
  _HLine(y1,x0,x1,color);
  _VLine(x0,y0,y1,color);
  _VLine(x1,y0,y1,color);
}
// -----------------------------------------------------------------
...
//    x, y  : 文本显示位置左上角象素坐标
// -----------------------------------------------------------------
void gTextOut(char *ch, u32 x, u32 y)
{
    u8 *pFont;
    int code,m,j,k;
    int x0=x,sx,sy; 
    ...
    LCD_Copy((u32*)LCD_GUI, x0,y,x-1,y+sy-1);
...
// -----------------------------------------------------------------
//	LCD fill a rectangle 
//  input : 
//    color_index = 8 bit color index to fill the rect
//    x0,y0 : upper-left conner
//    x1,y1 : lower-right conner
// -----------------------------------------------------------------
void LCD_FillRect(int x0,int y0,int x1,int y1, u32 color)
{
      int n,j,k ;
...
void LCD_Copy(u32 *pbuf, u32 x1,u32 y1, u32 x2, u32 y2)
{
	int n,j,k ;
	int p1;
...
{  
    ...
    while( (ryy*xx) < (rxx*yy) ){  
        ...
        if(fill){	// fill 
            if(di>=0){
                _HLine( wy0,wx0, wx1, color);
                _HLine( wy2,wx2, wx3, color);
            }
        }
            //
    } //end while
...
// -----------------------------------------------------------------
void _VLine(int x, int y0, int y1, u32 color)
{  
    int i;
    volatile u32 *p;
...
void _TextOut(u8 *ch,int x, int y)
{
        ...
        if(ch[k]>0xa0){	//hanzi
            ...
            for(m=0;m<FONTYSIZE;m++){
                ...
                for(j=0x80;j>0;j=j>>1){
                    //if(((*pFont)&j)!=0){
                    if((bm&j)!=0){
                        *(pLCD+(code)+x)=dc.color;
                    }
                    else{
                        *(pLCD+(code)+x)=dc.backcolor;
                    }
                    x++;
                }
                xe=x;
    ...
    //2013:
    if(xe>xs||ye>ys){
        LCD_Copy((u32*)LCD_GUI, xs, ys, xe, ye);
    }
...
// -----------------------------------------------------------------
//	LCD_GotoXY : 指定下一次字符输出位置(行列)
//  input : 
//    col : 列号，水平字符位置, 0-33
//    row : 行号，垂直字符位置, 0-19 
// -----------------------------------------------------------------
void LCD_GotoXY(u32 row, u32 col)
{
	dc.row=row;
	dc.col=col;
